...
def edit_task_interface(manager):
    ...
    try:
        ...
        new_title = input("  Новое название (оставьте пустым для без изменений): ").strip()
        
        print(f"  Текущее описание: {task.description or '(пусто)'}")
        new_description = input("  Новое описание (оставьте пустым для без изменений): ").strip()
        
        print("  Текущий приоритет:", {"low": "Низкий", "medium": "Средний", "high": "Высокий"}[task.priority])
        ...
        if manager.update_task(task_id, new_title or None, new_description or None, new_priority):
...
def delete_task_interface(manager):
    ...
    try:
        ...
        confirm = input(f"  Вы уверены, что хотите удалить задачу #{task_id}? (y/n): ").lower()
